...
add_zombie_weapon( weapon_name, hint, cost, weaponVO, variation_count, ammo_cost, box  )
{
	//MakeCents modified so dlc3_code editing for weapons is not needed if box is added to add_zombie_weapon below
	if(isdefined(box)){//do nothing different if you don't want to use this
		include_weapon(weapon_name,box);
	}else{
...
init_weapons()//MakeCents, add a var for box (true or false), must have ammo cost added too, 1/2 cost usually (do nothing different if you don't want to use this)
{//example: add_zombie_weapon( "colt", 									&"ZOMBIE_WEAPON_COLT_50", 					50,		"vox_crappy",	8, 25, false);
	// Zombify
